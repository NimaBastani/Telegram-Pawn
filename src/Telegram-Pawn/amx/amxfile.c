...
static char *completename(TCHAR *dest, TCHAR *src, size_t size)
{
    ...
    if (dest[len-1]!=__T('\\') && dest[len-1]!=__T('/') && len+1<size) {
      dest[len]=__T('\\');
      dest[len+1]=__T('\0');
    } /* if */
    ...
    for (ptr=src; *ptr!=__T('\0'); ptr++)
      if (ptr[0]==__T('.') && (ptr[1]==__T('\\') || ptr[1]==__T('/')))
